...
def pivot_tables(df: pd.DataFrame, group_cols: str, agg_cols: str, threshold_share: float = 0, plot: bool = False) -> pd.DataFrame:
    result_df = df.groupby([group_cols])[agg_cols].sum() \
        .to_frame("value") \
        .query("value > 0") \
    ...
    if plot is False:
        result_df = result_df.style.format({"value": '{:.1f}', "Total": '{:.1f}', "Share_of_total": '{:.1f}'})
    else:
        pass
    return result_df
